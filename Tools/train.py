...
#from core.criterion import OhemCEWithDiceEdge
...
def main():
    ...
    distributed = args.local_rank >= 0 and 'RANK' in os.environ and 'WORLD_SIZE' in os.environ
    ...
    if distributed:
        ...
        print(f"[INFO] Distributed mode initialized.")
    else:
    ...
    if config.TRAIN.FREEZE_EPOCHS > 0:
        print(f"[INFO] Freezing backbone for {config.TRAIN.FREEZE_EPOCHS} epochs.")
        for name, param in model.named_parameters():
            if not any(key in name for key in ['ocr', 'aux_head', 'cls_head']):
    ...
    if config.TRAIN.OPTIMIZER == 'sgd':
        ...
        if config.TRAIN.NONBACKBONE_KEYWORDS:
            ...
            params = [{'params': [p for p in model.parameters() if p.requires_grad], 'lr': config.TRAIN.LR}]
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if config.TRAIN.FREEZE_EPOCHS > 0:
            if epoch == config.TRAIN.FREEZE_EPOCHS:
                logger.info(f"Unfreezing Stage4 at epoch {epoch}")
                for name, param in model.module.model.named_parameters():
                    if 'stage4' in name:
                        param.requires_grad = True
                
                optimizer = torch.optim.SGD(
                    [{'params': [p for p in model.parameters() if p.requires_grad], 'lr': config.TRAIN.LR}],
                    lr=config.TRAIN.LR,
                    momentum=config.TRAIN.MOMENTUM,
                    weight_decay=config.TRAIN.WD,
                    nesterov=config.TRAIN.NESTEROV,
                )
                logger.info(f"Rebuilt optimizer after unfreezing Stage4")


            if epoch == config.TRAIN.FREEZE_EPOCHS + 10:
                logger.info(f"Unfreezing Stage3 at epoch {epoch}")
                for name, param in model.module.model.named_parameters():
                    if 'stage3' in name:
                        param.requires_grad = True
                ...
                logger.info(f"Rebuilt optimizer after unfreezing Stage3")

            if epoch == config.TRAIN.FREEZE_EPOCHS + 20:
                logger.info(f"Unfreezing full backbone at epoch {epoch}")
                for name, param in model.module.model.named_parameters():
                ...
                logger.info(f"Rebuilt optimizer after unfreezing all backbone")
        # =====================================
        ...
        valid_loss, mean_IoU, IoU_array = validate(config, testloader, model, writer_dict, epoch)
...
python Tools/train.py --cfg experiments/tbm/seg_hrnet_ocr_tbm.yaml
"""

"""
module load conda
conda activate tbview
tensorboard --logdir=/blue/pinaki.sarder/jingyixu/TBM/HRNet-Semantic-Segmentation/tbm/hrnet_ocr/ --port=6006
"""
"""
for i in {0..4}; do
  python Tools/train.py --cfg experiments/tbm/seg_hrnet_ocr_tbm.yaml \
    DATASET.TRAIN_SET cv_lst/tbm/folds/train_${i}.lst \
    DATASET.TEST_SET cv_lst/tbm/folds/val_${i}.lst \
    OUTPUT_DIR output/tbm_fold_${i}
done
"""