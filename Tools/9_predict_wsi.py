...
def postprocess_tbm(softmax_pred, threshold, min_area, raw_image):
    ...
    ### === [Color-based TBM Gap Filtering] === ###
    print("[Postprocess] Performing color-based filtering")

    # Convert raw image to RGB channels
    r, g, b = raw_image[..., 0], raw_image[..., 1], raw_image[..., 2]

    # Define white-looking pixel mask: all RGB > 160 (adjust as needed)
    white_mask = (r > 170) & (g > 170) & (b > 170)

    # Remove TBM pixels in white-looking regions
...
def main():
    ...
    logger.info(f"WSI size: {w}x{h}")
    print(f"[Main] Processing WSI of size {w}x{h}")
    ...
    for y in tqdm(range(0, h, stride), desc="[Main] Patch Y"):
        for x in range(0, w, stride):
            ...
            region = slide.read_region((x, y), 0, (x_end - x, y_end - y)).convert('RGB')
    ...
    raw_img = np.array(slide.read_region((0, 0), 0, (w, h)).convert("RGB"))
    final_tbm = postprocess_tbm(prob_map, threshold=args.threshold, min_area=args.min_area, raw_image=raw_img)
...
"""
python Tools/9_predict_wsi.py \
  --cfg /blue/pinaki.sarder/jingyixu/TBM/HRNet-Semantic-Segmentation/experiments/tbm/seg_hrnet_ocr_tbm.yaml \
  --image_path /blue/pinaki.sarder/jingyixu/TBM/HRNet-Semantic-Segmentation/data/WSIs/Image_DKD_1.svs \
  --output_path /blue/pinaki.sarder/jingyixu/TBM/HRNet-Semantic-Segmentation/Data/tbm/test/Image_DKD_1.png \
"""
